...
export default function Home() {
  ...
  const skillCategories = [
    { category: '言語', items: ['TypeScript', 'Go', 'JavaScript', 'Java', 'HTML', 'CSS'].sort() },
    { category: 'フレームワーク', items: ['React', 'Gin'].sort() },
    { category: 'データベース', items: ['PostgreSQL', 'SQLite'].sort() },
  ...
  const works = useMemo(() => [
    {
      ...
      tech: ['React', 'TypeScript', 'Go', 'SQLite'],
    ...
    {
      title: 'ほふりWebプロジェクト',
      description: '金融システムの改修。Web画面のデザイン改修およびJavaを用いたバックエンド改修を担当。',
      tech: ['HTML', 'CSS', 'JavaScript', 'Java'],
      startDate: '2022-07-01',
      endDate: '2023-10-31' as string | null,
      color: '#0891b2',
    },
  ], [])
  ...
  const { fullStart, fullEnd } = useMemo(() => {
    ...
    const ends = source.map(w => w.endDate ? new Date(w.endDate).getTime() : now.getTime())
    return {
      fullStart: new Date(Math.min(...starts)),
      fullEnd: new Date(Math.max(...ends)),
    }
  }, [filteredWorks, works, now])
  ...
  const chartWidth = Math.ceil((fullEnd.getTime() - fullStart.getTime()) / (1000 * 60 * 60 * 24) * pxPerDay) + 60 + CHART_PADDING
          ...
          <div className={styles.timelineChart}>
            <div style={{ width: `${chartWidth}px`, position: 'relative' }}>
              ...
              <div className={styles.timelineBars}>
                ...
                <div className={styles.todayLine} style={{ left: `${todayPx}px` }} />
                {filteredWorks.map(work => {
                    ...
                    <div key={work.title} className={styles.timelineRow}>
                      <div
                        ...
                        style={{
                          ...
                          opacity: hoveredWork === null || hoveredWork === work.title ? 1 : 0.3,
                          filter: hoveredWork === work.title ? 'brightness(1.3)' : 'none',
                          transition: 'opacity 0.2s, filter 0.2s',
                        }}
                      >
                        <span className={styles.timelineBarLabel}>{work.title}</span>
                      </div>
                    </div>
                  )
                })}
